...
namespace Geometry.Geometry3D
{
    ...
    /// </summary>
    public class Box : Polyhedron
    {
        public Box(Point center, double width, double length, double height)
        {
            Points.Add(center + new Vector(-width / 2, -length / 2, -height / 2));
            Points.Add(center + new Vector(width / 2, -length / 2, -height / 2));
            Points.Add(center + new Vector(width / 2, length / 2, -height / 2));
            Points.Add(center + new Vector(-width / 2, length / 2, -height / 2));
            Points.Add(center + new Vector(-width / 2, -length / 2, height / 2));
        ...
        public override IEnumerable<Plane> Faces
        {
            get
            {
                yield return new Plane(Points[0], Points[2], Points[1]);
                yield return new Plane(Points[0], Points[1], Points[5]);
                yield return new Plane(Points[1], Points[2], Points[6]);
                yield return new Plane(Points[2], Points[3], Points[7]);
                yield return new Plane(Points[3], Points[0], Points[4]);
                yield return new Plane(Points[4], Points[5], Points[6]);
            }
        }
        ...
        public override IEnumerable<Point> Vertices => Points;
